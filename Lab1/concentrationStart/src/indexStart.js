...
function showMatches() {
    ...
    status.innerText = `Matches: ${matches} Tries: ${tries}`;
}
...
function fillImages() {
    ...
    for (var value of values) {
        for (var suit of suits) {
            ...
            images[index++] = `card${value}${suit}.jpg`; // Add pairs
        }
    }
}
...
function showBack(index) {
    ...
    card.style.backgroundImage = `url('${imagePath}cardback.jpg')`;
...
function handleClick() {
    ...
    var cardImage = images[index];
    // set the backgroundImage to the url of the cardImage
    this.style.backgroundImage = `url('${imagePath}${cardImage}')`;
